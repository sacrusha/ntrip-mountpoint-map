...
The **user-facing translation.** Must read docs/target-users.md to understand who users are.
Subset of networks worth a country-level marker. Renders directly in map popups for end users.
...
`vrs: true` set iff the network delivers VRS streams. Independent of tier.
...
- **Nothing-here guard.** If the only thing to say is "no service exists", omit the marker. Empty space beats a tag that just confirms absence.

## Note anti-patterns (beyond JSON conventions)

- No Harassment: No email, phone, named individual, bank/giro details, or "contact X"
  instructions - link via `registration` url instead.
- No developer facing language ("No explicit restriction found", PDF dates).
