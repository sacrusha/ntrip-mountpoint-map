...
Tier assignment derives from `status` in `docs/networks.md`:

- `in-pipeline` + VRS-only → coloured circle (same colour as source in toggle panel)
- `deferred` (free; endpoint withheld or not found) → grey circle
- `paid-affordable` (under $200/yr cutoff; currently HEPOS/GR, ROMPOS/RO) → **?** marker
- `paid` or `rejected` (policy / restriction reason) → **?** marker
...
Data backing: `data/country_markers.json` — static file, 104 entries. Fields:
`id`, `name`, `region`, `country` (ISO 3166-1 α-2), `lat`, `lon`, `tier`,
`source_id` (vrs tier only — links to `stations.json` for colour and
...
Frontend suppresses a vrs circle when `sources[source_id].stations.length > 0`
(network regained physical pins). Maintained by hand; not generated by the
pipeline.
