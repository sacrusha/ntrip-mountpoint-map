...
**Date researched:** 2026-05-21 (station list research added; prior: 2026-05-17 deep verification)
...
|---|---|---|---|---|
| BB (Brandenburg) | 22 | `https://geobasis-bb.de/lgb/de/geodaten/raumbezug-sapos/sapos-referenzstationen/` — full table 22 stations + Gauss-Krüger coords | ETRS89 lat/lon via sitelogs at `https://monitor.sapos-bb.de/station/sitelogs/` (22 files, naming `00XXXX DEU_YYYYMMDD.log`) | Sitelog access confirmed 2026-05-21; DMS format (+HHMMSS.ss); format: `gnss-metadata.eu` style |
...
| BW (Baden-Württemberg) | 16 | Metadata record at `https://metadaten.geoportal-bw.de/geonetwork/srv/api/records/66a92120-7998-e9a2-9eb1-8dd4f997e755`; interactive refmap `https://gpps-web.sapos-bw.de/karte.php` | No: refmap is dynamic JS rendering only; static ETRS89 coord list not published; station IDs visible include MANN, SHA2, BRML, WEIN | Contact `benutzerservice@lgl.bwl.de` for sitelog archive; ZSS Technology FAQ states sitelogs available centrally |
| HE (Hessen) | unknown | `https://sapos.hvbg.hessen.de/refmap.php` — interactive refmap with per-station detail on click | No public static coord list; refmap uses TLS with expired/self-signed cert (unreachable from sandbox) | Customer newsletter references Fulda, Fulda2, Neuhof station names |
| NI+HB (Niedersachsen+Bremen) | 43 | No public list or refmap found | No: LGLN does not publish a station coordinate table; 10 station IDs known from May 2025 rename notice: OSNA, AURI, CLA2, HAM2, CADE, CARO, ESTE, GNA2, VERD, MEPP | GPPS portal `kundenportal.openservice-sapos.niedersachsen.de/gppweb/rinex` allows selection by station but requires registered credentials |
| SN (Sachsen) | 19 | Network map PDF: `https://www.landesvermessung.sachsen.de/webanwend/Vernetzung-Sachsen_2023.pdf` (too large to parse; image-based) | No public ETRS89 coord list found; GeoSN SAPOS webserver at `landesvermessung.sachsen.de/sapos/` likely has detail but requires login | Zinnwald named as first of five ground-edge stations built 2020; Dresden, Leipzig, Chemnitz area confirmed as station locations from survey literature |

**Conclusion for map pipeline:** Brandenburg (BB) is the only one of these six SAPOS casters where a practical path to ETRS89 lat/lon is confirmed via public sitelogs. The other five (BE, BW, HE, NI, SN) either have 0 pins to add (BE) or require credentials / dynamic rendering / oversized PDF to retrieve coordinates.
