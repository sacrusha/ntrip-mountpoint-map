...
**Date researched:** 2026-05-21 (BB sitelogs + NRW refmap detail pages confirmed as pinnable coord sources; HE/MV/SN/NI/BW dead-ends re-verified; prior: 2026-05-17 deep verification)
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| Baden-Württemberg | BW | LGL BW | `www.sapos-bw-ntrip.de:2101` | https://www.lgl-bw.de/unsere-themen/Geoinformation/Geodaetischer-Raumbezug/Satellitenpositionierungsdienst/ | https://www.lgl-bw.de/unsere-themen/Geoinformation/Geodaetischer-Raumbezug/Satellitenpositionierungsdienst/Registrierungsformular/index.html | Data free ("Open SAPOS®, vollständig gebührenfrei"); €150 incl. USt one-time admin/credential fee retained per HEPS Zugangskennung; GPPS data free | VRS_3_2G_BW · VRS_3_3G_BW · VRS_3_4G_BW | 16 (per geoportal-bw metadata record and LGL statement "aktuell 16 landesweit verteilte Referenzstationspunkte"; station IDs visible on refmap include MANN, SHA2, BRML, WEIN among others; no public ETRS89 coord list found — coords not exposed on `gpps-web.sapos-bw.de/karte.php`) | yes | unclear | 2026-05-07 |
| Bayern | BY | LDBV | `www.sapos-by-ntrip.de:2101` | https://sapos.bayern.de/ | https://sapos.bayern.de/download.php?file=SAPOS-Nachrichten-2024_01.pdf | €20/yr/account (incl. HEPS + GPPS; flat rate since 01.06.2024; no per-use charges; excl. VAT) | VRS_3_2G_BY · VRS_3_3G_BY · VRS_3_4G_BY | 39 confirmed (refmap `sapos.bayern.de/refmap.php` lists 39 named stations including München, Augsburg, Garmisch, Erlangen etc.; LDBV press materials sometimes say "rund 50" including cross-border neighbours) | yes | unclear | 2026-05-07 |
| Berlin | BE | SenStadt Berlin | `www.sapos-be-ntrip.de:2101` | https://www.berlin.de/sen/sbw/stadtdaten/geoportal/landesvermessung/raumbezug/sapos/ | https://www.berlin.de/sen/sbw/stadtdaten/geoportal/landesvermessung/raumbezug/sapos/ | Free | VRS_3_2G_BE · VRS_3_3G_BE · VRS_3_4G_BE | 0 own stations (Berlin is a city-state; all physical CORS are owned/maintained by neighbouring BB + ST; caster routes to those — no Berlin-owned stations to enumerate) | yes | unclear | 2026-05-07 |
| Brandenburg | BB | LGB (Potsdam) | `www.sapos-bb-ntrip.de:2101` | https://geobasis-bb.de/lgb/de/geodaten/raumbezug-sapos/ | https://geobasis-bb.de/lgb/de/geodaten/raumbezug-sapos/ | Free (since 2020) | VRS_3_2G_BB · VRS_3_3G_BB · VRS_3_4G_BB | 22 (LGB station list at `geobasis-bb.de/lgb/de/geodaten/raumbezug-sapos/sapos-referenzstationen/`; sitelogs with ETRS89 lat/lon at `monitor.sapos-bb.de/station/sitelogs/`) | yes | unclear | 2026-05-07 |
...
| Sachsen | SN | GeoSN | `www.ntrip.sachsen.de:2101` | https://www.landesvermessung.sachsen.de/sapos-sachsen-7213.html | https://www.landesvermessung.sachsen.de/sapos-sachsen-7213.html | Free (since Open Data rollout 2019; costs "fully eliminated") | VRS_3_2G_SN · VRS_3_4G_SN · FKP_3_2G_SN · MAC_3_2G_SN | 19 (GeoSN states "19 SAPOS-Stationen in Sachsen"; plus 17 cross-border stations from PL/CZ/neighbouring Länder integrated; named stations include Zinnwald "erste SAPOS-Bodenstation" inaugurated 2020-05-18; network diagram PDF `Vernetzung-Sachsen_2023.pdf` at landesvermessung.sachsen.de/webanwend/ is oversized for retrieval; no public ETRS89 coord list found) | yes | unclear | 2026-05-07 |
...
## Physical Station Lists — Sources for Map Pins

The SAPOS casters publish only VRS/FKP/MAC mountpoints with no per-station lat/lon in the sourcetable. Physical station coordinates must be sourced separately from the operator portals. Research conducted 2026-05-21.

| State | Num stations | Station list URL / source | Coordinates available? | Notes |
|---|---|---|---|---|
| BB (Brandenburg) | 22 | `https://geobasis-bb.de/lgb/de/geodaten/raumbezug-sapos/sapos-referenzstationen/` — full table 22 stations + Gauss-Krüger coords | **Yes — pipeline-wired.** ETRS89 lat/lon via sitelogs at `https://monitor.sapos-bb.de/station/sitelogs/` (22 IGS-format `.log` files, naming `00XXXX DEU_YYYYMMDD.log`). DMS strings `+DDMMSS.ss` in Section 2. | Scraped weekly via `scripts/scrapers/sapos_bb.py`; cache at `data/sapos_BB_ext.scraped.json`. |
| BE (Berlin) | 0 own | No station list — Berlin is a city-state with no SAPOS-owned CORS; all physical bases in the BB/ST caster pool | n/a — nothing to pin | Berlin caster routes computations to BB + ST stations; map pins should come from BB/ST entries |
| BW (Baden-Württemberg) | 16 | Metadata record at `https://metadaten.geoportal-bw.de/geonetwork/srv/api/records/66a92120-7998-e9a2-9eb1-8dd4f997e755`; interactive refmap `https://gpps-web.sapos-bw.de/karte.php` | **No (dead-end).** Refmap is dynamic JS rendering only; static ETRS89 coord list not published. Station IDs visible in JS include MANN, SHA2, BRML, WEIN. | Contact `benutzerservice@lgl.bwl.de` for sitelog archive; ZSS Technology FAQ states sitelogs available centrally but not at a public URL. |
| HE (Hessen) | unknown | `https://sapos.hvbg.hessen.de/refmap.php` — interactive refmap with per-station detail on click | **No (dead-end, 2026-05-21).** TLS cert chain incomplete (re-verified via WebFetch); both browser-class and `urllib` fetches reject. Even on cert override the refmap renders dynamically. | Customer newsletter references Fulda, Fulda2, Neuhof station names. |
| MV (Mecklenburg-Vorpommern) | 17 | LAiV publishes `Uebersicht_Referenzstationen_2025_Juli.pdf` at `laiv-mv.de/static/LAIV/Geoinformation/Dateien/Raumbezug/` | **No (dead-end, 2026-05-21).** PDF is image-based (no text layer; binary returned by WebFetch). LAiV download portal at `laiv.geodaten-mv.de/afgvk/Raumbezug/Beschreibung?produkt=SATPOSDIENSTE` exposes no machine-readable station list. The NTRIP `:2102` `_phy` endpoint publishes RAW-format only (17 stations RAW_0773–RAW_0809), all dropped by the post-process filter. | If LAiV ever publishes a text-extractable list, a scraper similar to `sapos_bb.py` would supply pins for the 17 RAW_XXXX stations — names are operator-known, just not joined to coords publicly. |
| NI+HB (Niedersachsen+Bremen) | 43 | No public list or refmap found | **No (dead-end).** LGLN does not publish a station coordinate table; 10 station IDs known from May 2025 rename notice: OSNA, AURI, CLA2, HAM2, CADE, CARO, ESTE, GNA2, VERD, MEPP. EPN sitelog API at `gnss-metadata.eu/v1/sitelog/exportlog?id=XXXX00DEU` also fails with TLS cert error from sandbox (`ERR_TLS_CERT_ALTNAME_INVALID`). | GPPS portal `kundenportal.openservice-sapos.niedersachsen.de/gppweb/rinex` allows selection by station but requires registered credentials. |
| NW (Nordrhein-Westfalen) | 27 (refmap list, vs LGLN's "~37" PR figure) | `https://gppspro.saposnrw.de/refmap.php` — index of 27 named stations; each links to `refmap.php?detail=<ID>` carrying the embedded IGS sitelog | **Yes — pipeline-wired.** Detail pages publish DMS coords in standard IGS sitelog Section 2 format. Confirmed 2026-05-21: BONN, WESEL, VIERSEN, COESFELD, ESSEN, MUENSTER, ARNSBERG, VREDEN, AACHEN, MECHERNICH, MINDEN, GUMMERSBACH, WINTERBERG, LICHTENAU, BIELEFELD, RHEINE, LIPPSTADT, DETMOLD, FINNENTROP, HAGEN3, HAMM2, SIEGEN2, KLEVE2, KOELN2, SELFKANT2, TITZ2, DUESSELDORF2. | Scraped weekly via `scripts/scrapers/sapos_nw.py`; cache at `data/sapos_NW_ext.scraped.json`. The 27 vs 37 gap likely reflects neighbouring-Land partner stations integrated into the NRW solution that Geobasis NRW does not own — typical for AdV cross-border vernetzung. |
| SN (Sachsen) | 19 | Network map PDF: `https://www.landesvermessung.sachsen.de/webanwend/Vernetzung-Sachsen_2023.pdf` (too large to parse; image-based) | **No (dead-end).** No public ETRS89 coord list found; GeoSN SAPOS webserver at `landesvermessung.sachsen.de/sapos/` requires login. | Zinnwald named as first of five ground-edge stations built 2020; Dresden, Leipzig, Chemnitz area confirmed as station locations from survey literature. |

**Conclusion for map pipeline:** Brandenburg (22 stations) and Nordrhein-Westfalen (27 stations) are pipeline-wired as of 2026-05-21 via `type: "scraped"` endpoints in `data/rtk_map.json`. Dead-ends (BW, HE, MV, NI, SN) require credentials, dynamic JS rendering, image-PDF OCR, or a working EPN API; none of those reduce cleanly to a small scraper module. BE has 0 own stations.
...
- SAPOS BB station list (22 stations, Gauss-Krüger coords): https://geobasis-bb.de/lgb/de/geodaten/raumbezug-sapos/sapos-referenzstationen/ (verified 2026-05-21)
- SAPOS BB sitelogs with ETRS89 lat/lon (22 .log files, updated 2026-03-10): https://monitor.sapos-bb.de/station/sitelogs/ (verified 2026-05-21; format `000XXXDEU_YYYYMMDD.log`)
- SAPOS BW station metadata (16 RSP, geoportal-bw GeoNetwork): https://metadaten.geoportal-bw.de/geonetwork/srv/api/records/66a92120-7998-e9a2-9eb1-8dd4f997e755 (verified 2026-05-21)
- SAPOS BW refmap (dynamic JS, station IDs MANN/SHA2/BRML/WEIN visible): https://gpps-web.sapos-bw.de/karte.php (verified 2026-05-21, no static coord extraction possible)
- SAPOS HE refmap: https://sapos.hvbg.hessen.de/refmap.php (SSL cert issue, inaccessible from sandbox 2026-05-21)
- SAPOS NI 43 stations confirmed + 10 renamed 2025-05-22 (OSNA AURI CLA2 HAM2 CADE CARO ESTE GNA2 VERD MEPP): https://lgln-geodaten.niedersachsen.de/startseite/sapos_raumbezug/sapos/sapos-r-dienste-143814.html (verified 2026-05-21)
- SAPOS SN 19 stations confirmed, Zinnwald "first ground station": https://www.geosn.sachsen.de/einweihung-erste-sapos-bodenstation-5015.html (verified 2026-05-21)
- SAPOS SN Vernetzung-Sachsen_2023.pdf (network map, image-based PDF): https://www.landesvermessung.sachsen.de/webanwend/Vernetzung-Sachsen_2023.pdf
