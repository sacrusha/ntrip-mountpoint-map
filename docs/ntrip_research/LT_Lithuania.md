...
| **host:port — primary** | `193.219.10.2:2101` (VilniusTech Geodesy Institute, Vilnius) — confirmed live 2026-05-07: `SOURCETABLE 200 OK ENDSOURCETABLE` from `Trimble Pivot Platform`, 12 STR rows |
| **host:port — secondary** | `195.182.72.152:2101` (GIS-Centras / SSVA) — confirmed live 2026-05-07: `SOURCETABLE 200 OK`, identical 12 STR rows. Both servers provide identical streams; no DNS hostname is published for either IP. Some user docs additionally cite alternate port `2111` |
...
| **last_confirmed_alive** | 2026-05-07 — `193.219.10.2:2101` and `195.182.72.152:2101` both returned `SOURCETABLE 200 OK` (Trimble Pivot Platform), 12 STR rows each; geoportal.lt LitPOS-EN portal accessible (About / Status / Application / Registration / Contacts pages all reachable) |

## Mountpoint Catalogue — both servers (sourcetable 2026-05-07)
...
- **Stations.json 2026-05-06 fetch**: LT = 1 Centipede entry only (volunteer); LitPOS itself contributes 0 mappable pins given the VRS-only sourcetable.
...
- LitPOS English about page: https://www.geoportal.lt/geoportal/web/litpos-en (full text extracted 2026-05-07: "Total number of LitPOS GNSS stations is 35", "users can use 3 ASG-EUPOS Polish stations and 6 LATPOS Latvian stations")
...
- Live primary sourcetable: `curl http://193.219.10.2:2101/` → `SOURCETABLE 200 OK ENDSOURCETABLE` Server: `Trimble Pivot Platform`, 12 STR rows (2026-05-07)
- Live secondary sourcetable: `curl http://195.182.72.152:2101/` → identical structure (2026-05-07)
...
- Stations.json 2026-05-06 fetch: LT = 1 Centipede entry; LitPOS contributes 0 mappable pins (vrs-only)
