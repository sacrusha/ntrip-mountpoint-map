...
**Date researched:** 2026-05-22 (re-verified — no operational change) | Currency: XAF (Central African CFA franc, CEMAC zone) — fixed peg €1 = 655.957 XAF
...
None found. No published project timeline for any CAR CORS/NTRIP service exists as of 2026-05-22. The Ministry of Town Planning (`minurbanisme-rca.org`) lists only land-reform, housing, and Bangui-district rehabilitation programmes — no satellite-positioning, geodesy, or CORS line items.

**AFREF (regional context):** Each AFREF participating country is expected to operate at least one continuously transmitting station. CAR remains absent from AFREF's operational contributing countries; the GIM International "Fully Fledged CORS Map for Africa" article (re-checked 2026-05-22) reconfirms CAR is neither in the 25 mapped-CORS countries nor in the named "unmapped-with-known-network" list. No CAR station in IGS Network as of 2026-05-22.
...
- **DePIN networks**: GEODNET, ONOCOY, Centipede-RTK, RTKdata — none report any CF coverage as of 2026-05-22.
- **Local pipeline data (verified 2026-05-22)**: `py scripts/stations_by_country.py CAF` returns `No stations for 'CAF'`. `py scripts/stations_by_radius.py 4.39 18.55 600` returns `No stations within 600 km of (4.39, 18.55)`. Nearest pipeline-known stations are EarthScope RUBO/KMBR/NYBA in Rwanda at ~1370 km — far outside any usable RTK baseline.
...
- Local pipeline: `data/stations.json` via `stations_by_country.py CAF` and `stations_by_radius.py 4.39 18.55 600`
