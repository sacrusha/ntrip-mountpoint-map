...
**Date researched:** 2026-05-17 (reverification — cors_korea pipeline still 493 stations after de-dup; rtk2go KR still 2; NGII English service-info page returns HTTP 404 on WebFetch; gnssdata.or.kr:2101 not reachable from sandbox 2026-05-17 but pipeline last fetched it successfully)
**Prior rounds:** 2026-05-12 (rtk2go count correction); 2026-05-08 (commercial-tier sweep); 2026-05-07
...
| **landing_url — Network 1** | https://www.gnssdata.or.kr/ (operator-owned portal landing) |
| **access_url — Network 1** | https://www.gnssdata.or.kr/user/agree.do (distinct self-serve registration page; RTCM service-info / one-click apply at https://www.gnssdata.or.kr/rtcm/getRtcmView.do) |
| **num_stations — Network 1** | 167 unique physical stations (de-duped by base-code prefix from 546 STR rows in live sourcetable 2026-05-08; data/stations.json `cors_korea` pipeline parse yields 493 distinct mountpoints across format variants). 8 contributing agencies: NGII (KORREF, 94), Seoul Metropolitan Government (SMG, 10), plus 433 Single-Base rows from KASI and other operators. |
| **vrs — Network 1** | no — single-base raw RTCM observations (rover computes RTK baseline); not VRS / MAC / FKP / i-MAX. |
...
| **last_confirmed_alive — Network 1** | 2026-05-17 -- `data/stations.json` `cors_korea` source still carries 493 de-duped stations from `www.gnssdata.or.kr:2101`; last direct `curl --http0.9 http://www.gnssdata.or.kr:2101/` from sandbox returned HTTP 000 (likely sandbox egress), but RTCM service page reached HTTP 200 in prior round (2026-05-08) and pipeline fetch remains current. Verbatim from 2026-05-08 RTCM page: `Caster Host : www.gnssdata.or.kr, 포트 : 2101, User ID : 로그인 아이디, 비밀번호 : gnss`. |
| **datum_epoch -- Network 1/2** | omitted -- no operator-portal declaration of frame/epoch surfaced in `gnssdata.or.kr` registration / RTCM-service pages or NGII portal pages reviewed 2026-05-17 (NGII English Network-RTK service-info page `eng/contents/contentsView.do?rbsIdx=174` returns HTTP 404). Korea's national frame KGD2002 = ITRF2000 @ 2002.0 is widely known but is not operator-NTRIP-portal-cited per primer rule. |
...
| **landing_url — Network 2** | https://map.ngii.go.kr/ms/mesrInfo/gnss/vrsUserView.do (operator-owned NGII Network RTK service page) |
| **access_url — Network 2** | https://www.ngii.go.kr/member/login.do (distinct unified-portal member registration; Korean PASS / mobile identity verification required) |
| **num_stations — Network 2** | 94 NGII KORREF physical stations (count from cors_korea sourcetable agency-tag breakdown 2026-05-08); NGII operator material describes ~60 stations at ~40 km spacing for the network-RTK solution — the difference reflects post-expansion KORREF additions whose subset is used in the VRS/FKP solution. |
| **vrs — Network 2** | yes — VRS (Geo++ GNSMART) and FKP corrections; cm-level network solution. |
...
| **last_confirmed_alive — Network 2** | NGII unified portal `www.ngii.go.kr` HTTP 200 (2026-05-08); 2022-05-02 address-change notice (`sq=77502`) still referenced; English Network-RTK service-info page (`eng/contents/contentsView.do?rbsIdx=174`) HTTP 404 on WebFetch 2026-05-17 (consistent with prior 2026-05-08 observation). Korean-language guides (e.g. `koseco.co.kr/ngii_rtk/`) continue to point at `vrs3.ngii.go.kr:2101` + `fkp.ngii.go.kr:2201`. Direct anonymous ST curl not feasible -- NGII VRS requires authenticated CONNECT. |
...
- **Pipeline coverage** (stations.json 2026-05-17 reverification via `py scripts/stations_by_country.py KOR`): KR = 493 cors_korea entries returned in helper script output (cors_korea pipeline source corresponds to `www.gnssdata.or.kr:2101` / Network 1). 493 ≠ 546 because the pipeline parser de-duplicates per `(base, lat, lon)` even where multiple format variants exist. rtk2go KR count = 2 (KAIST-RTK-RTCM at 36.37 N / 127.37 E; TYMICT-ES at 37.41 N / 127.13 E, last confirmed 2026-05-12); centipede KR = 0. *(Earlier file note mentioned a third rtk2go mountpoint `sejongnav` — not present in current stations.json; either decommissioned or never tagged KR.)*
