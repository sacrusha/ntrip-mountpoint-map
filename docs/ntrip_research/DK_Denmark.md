...
## Status: NO free public NTRIP — three Klimadatastyrelsen-registered commercial providers (GPSnet.dk, HxGN SmartNet, RTKconnect.dk); state operates 15 reference stations (expanded 14→15 in April 2025 with Hanstholm addition) but operates no free public caster. Free RINEX archive via Dataforsyningen. Sparse volunteer alternatives: 17 rtk2go DK bases + 17 Centipede DK nodes (split across non-ISO codes `DNK` 8 + `DAN` 9; see `_centipede_country_codes.md`) per sourcetable 2026-05-19
...
| **hobbyist_eligibility** | **?** — multiple subscription tiers including "drone" and "research and development"; no explicit licence requirement stated but not verified |
...
| **tariff** | **6,599 DKK + moms / year** (effective gross ~8,249 DKK incl. 25% moms); **19,797 DKK + moms / 3 years** (incl. support contract valued at 7,164 DKK ex-moms; effective gross ~24,746 DKK); volume pricing 2,999 DKK/user/year ex-moms for 11+ simultaneous users (effective gross ~3,749 DKK/user). Danish moms 25%. Date observed: 2026-05-21 (re-verified). Source: https://rtkconnect.dk/products/rtk-netvaerk |
| **hobbyist_eligibility** | **Yes** — single-login per subscription, unlimited devices; no licence requirement stated |
| **legal_residency_required** | **Unclear** |
| **last_confirmed_alive** | rtkconnect.dk accessible 2026-05-21; service product page still shows the same 6,599 DKK / 19,797 DKK / 2,999 DKK tiering and 100% uptime claim; Klimadatastyrelsen approval timestamped 17 November 2023 |

- **Stations:** ~111 base stations including 13 from Klimadatastyrelsen per RTKconnect's product page (KDS state network expanded to 15 stations in April 2025; RTKconnect's figure of 13 may predate the Hanstholm addition); avg baseline 10 km
...
| Option | Notes |
|---|---|
| **Centipede** (crtk.net) | **17 Denmark-coded nodes total** as of sourcetable 2026-05-19, split across **two parallel Centipede country codes**: 8 `DNK`-coded (`AGBI`, `AGLU`, `AGRB`, `AGSA`, `AGSB`, `AGTH`, `HZAG`, `OVTA`) and 9 `DAN`-coded. All cluster in Jutland (54.9–57.4°N, 8.5–10.3°E); near-zero coverage in Sjælland east. The shared `AG*` prefix on most nodes suggests a coordinated agricultural-farmer deployment — almost certainly one network with a labelling inconsistency in the Centipede sourcetable. **`DAN` is a non-ISO Centipede code in parallel with `DNK`; both mean Denmark** — see `_centipede_country_codes.md`. |
...
## Sources Consulted
- Klimadatastyrelsen GNSS positioning services: https://www.klimadatastyrelsen.dk/kortlaegning/geodaesi/gnss-positioneringstjenester (re-verified 2026-05-21 — 3 registered providers: Geoteam/GPSnet.dk (Sep 2008), Leica Geosystems / HxGN SmartNet (Sep 2008), RTKconnect (2024); state monitors, does not operate)
...
- RTKconnect pricing: https://rtkconnect.dk/products/rtk-netvaerk (re-verified 2026-05-21 — 6,599 DKK + moms / 1-year; 19,797 DKK + moms / 3-year incl. support contract valued at 7,164 DKK ex-moms; volume 2,999 DKK/user/year ex-moms for 11+ users; Klimadatastyrelsen approval 17 November 2023)
...
- Project pipeline `scripts/stations_by_country.py DNK` and `py scripts/stations_by_country.py DAN` (2026-05-19 sourcetable): 17 rtk2go DNK bases + **17 Centipede nodes total = 8 DNK + 9 DAN** (Centipede uses non-ISO `DAN` in parallel with `DNK` for Denmark; both must be summed). See Free/Volunteer table above and `_centipede_country_codes.md`.
