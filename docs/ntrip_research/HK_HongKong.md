...
| **host:port** | `ntrip.geodetic.gov.hk:2101` (new domain active from 1 June 2023; legacy `www.geodetic.gov.hk:2101` also documented; sourcetable at `http://183.178.46.138:2101/sourcetable.htm`) |
...
| **last_confirmed_alive** | 2026-05-06 (geodetic.gov.hk/en/satref/Net_RTK.htm loaded and returned current connection settings; NTRIP page HTTP 200 confirmed) |

## Service Details

### Network RTK (centimeter-level)
SatRef Network RTK uses 19 Continuously Operating Reference Stations (CORS): 16 reference stations plus 3 Integrity Monitoring (IM) stations evenly distributed across Hong Kong. The network delivers centimeter-level corrections via NTRIP using the VRS method. RTCM format mountpoints available (see sourcetable).
...
- **TCHK maintenance**: The Tate's Cairn Integrity Monitoring Station (TCHK) has been under maintenance since 5 August 2025; its Network RTK Integrity Monitoring function and GNSS Raw Data Streams are suspended until further notice. The main RTK correction service across the remaining 18 stations continues to operate.
