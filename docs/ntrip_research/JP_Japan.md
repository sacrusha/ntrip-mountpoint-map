...
| **MIRAI account validity** | Accounts expire after 365 days inactivity (per existing rtk_inventory.md note); attribution required: *"Source: GO!GNSS GO!JAPAN website (URL)"* etc. |
...
- Existing rtk_inventory.md `mirai` entry (~325 stations, NULA-style auth, 365-day inactivity expiry)
- Existing rtk_inventory.md `geortk` entry (`nmea_filter=False, solution_filter=False`, ~41 stations historic — now expanded to ~500+ per current site stamp)
