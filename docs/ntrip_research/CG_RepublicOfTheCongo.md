...
**Date researched:** 2026-05-22 (re-verified — no operational change. CERGEC / IGN FI 2019 news wire reconfirmed; no later CORS deployment reported.)
...
**CERGEC / IGN FI geomatics cooperation (26 June 2019, resumption of prior agreement)**: In Brazzaville, IGN FI regional director Aude Areste announced support for CERGEC (Centre de Recherche Géographique et de Production Cartographique). Scope: cartographic document digitisation, satellite imagery / remote sensing, GIS training, 1:50 000 map renewal. **No CORS network, NTRIP caster, or permanent GNSS station is named, dated, or budgeted.** Re-confirmed 2026-05-22: no later deployment reported.
...
## Verification Probes (2026-05-22)

- **rtk2go sourcetable** — 0 COG mountpoints (live probe 2026-05-22).
- **Centipede-RTK sourcetable** — 0 COG mountpoints (live probe 2026-05-22).
- **IGS network** — 0 CG stations.
- **SONEL** — 0 GNSS stations in CG.
- **Local data/stations.json** — `py scripts/stations_by_country.py COG` → none; `py scripts/stations_by_radius.py -4.27 15.28 600` → none.
- **GEODNET, Onocoy, RTKdata.online** — no CG coverage.
- **GIM International "Fully Fledged CORS Map for Africa"** (re-checked 2026-05-22) — Congo-Brazzaville not in the 25 mapped-CORS countries.

## Nearest Cross-Border Alternative

Nothing within ~50 km of any Congolese population centre. The nearest IGS permanent station is **NKLG (Libreville, Gabon)** at (0.354, 9.672), ~808 km NW of Brazzaville (haversine; bearing ~309°). Rebroadcast on AUSCORS / IGS-IP / MIRAI casters but at this baseline it is unusable for RTK — and at ~800 km it is also outside the useful range of any single-base long-baseline processing strategy; only PPP remains.
...
| **CDDIS / IGN data centres** | https://cddis.nasa.gov/ | IGS RINEX hosting for NKLG (Gabon, ~808 km NW of Brazzaville) — at this distance even static post-processing baselines are marginal; PPP is the dominant practical fallback. |
...
- IGS network (https://network.igs.org/) — 0 CG stations, 2026-05-22
