...
## Status: MIXED — free public casters: AR (ARDOT), AZ (AZCORS), CO (Mesa County RTVRN), OR (ORGN); paid subscription casters: LA (C4Gnet), UT/NV (TURN GPS), MT (MTSRN), WA (WSRN); TX restricted to DOT employees; OK, NM, ID, HI have no state caster; CA (CRTN) has a one-time fee model; EarthScope NOTA + NPS CORS provide free single-base fallback region-wide
...
- **EarthScope NOTA** — `ntrip.earthscope.org:2101` — free non-commercial, single-base streams only. Densest coverage in seismically active western Cordillera (PBO heritage).
...
| **Western coverage** | Yosemite, Yellowstone, Grand Canyon, Glacier, Olympic, Crater Lake, Death Valley, Joshua Tree, Hawaii Volcanoes, Haleakalā, Bryce Canyon, Zion, Sequoia, and others. Pacific stations include Hawaii Volcanoes (HAVO — flagged offline 2026-05-02), Haleakalā (HALE — flagged offline). Datum transitioning to MYCS3 (applied 2026-02-13). |
...
No state-operated RTK NTRIP caster confirmed operational for New Mexico as of 2026-05-07. NMDOT does not appear to operate a public CORS RTK network. Multiple sources (GPS World, E38 Survey Solutions, Point One Nav) do not list New Mexico as having a public service.

**Historic ARTGN (Albuquerque Real-Time GNSS Network):** Launched 2007 by the City of Albuquerque as a paid subscription service (~USD 200/month per 2010 American Surveyor article); it served the Albuquerque Geodetic Reference System (AGRS) ~800-monument framework. Current operational status is undocumented online — no recent press releases or municipal updates found post-2013. The City of Albuquerque AGRS page at cabq.gov/municipaldevelopment/architects-engineers-contractors/construction-services/albuquerque-geodetic-reference-system makes no current ARTGN reference. Contact City Surveyor Loren Risenhoover at 505-768-3614 to confirm current status.

**Gap:** Commercial options: RTKdata (USD 40/mo, claims statewide NM coverage), Point One Polaris, RTK Premium. EarthScope NOTA has stations in NM (instrument density is moderate along the Rio Grande Rift); useful for PPK and possibly for single-base RTK near population centers (TUCUMCARI, PIETOWN, WHITE SANDS region).
...
- Commercial alternative: AZGPS (azgps.net) — paid VRS network also covering AZ; founded Nov 2004; subscription pricing not publicly listed.
...
| **tariff** | Paid — USD 600/year per login (currently includes access to both TURN and Nevada GPS networks; UGRC has indicated this may split per-region in the future); payment via turngps-billpay.ugrc.utah.gov |
...
- Constellations: GPS + GLONASS + Galileo + BeiDou (full GNSS via RTCM32).
- TURN GPS also administers the Nevada GPS Network (Reno area) — see NV section below.
...
| **Operator** | Las Vegas Valley Water District (in cooperation with Nevada DOT, City of Las Vegas, Clark County Water Reclamation District, Lincoln County NV) |
| **host:port** | host not publicly listed; **port 9899**. Mountpoint names match site names (e.g. `nvbm`). RTK data link via NTRIP or TCP/IP. (Source: lvvwd.com/engineering-resources/survey-right-of-way/index.html, observed 2026-05-07) |
| **tariff** | Not publicly listed — credentials issued by the District Surveyor on contact (702-258-7163) or via account request form at lvvwd.com/apps/base-station-network-access/ |
...
- LVVWD covers the Las Vegas Valley with its own station network distinct from UGRC; access is application-based, pricing is not disclosed publicly.
- EarthScope NOTA has stations in Nevada (Basin and Range region, high station density compared to many western states) — free fallback.

---

## WY — Wyoming: No Public State Caster

No state-operated RTK NTRIP caster found for Wyoming as of 2026-05-07. WYDOT does not operate a public CORS RTK network. The Professional Land Surveyors of Wyoming (PLSW) references CORS resources but does not operate an NTRIP caster. The Wyoming Geodetic Coordination Committee (geodetic.geospatialhub.org) lists resources but no operational caster.
...
| **tariff** | Paid — USD 1,500 per login per year (rate effective July 1, 2024; rates reviewed each biennium and announced January of odd-numbered years, taking effect July 1; no 2026-07-01 update yet announced as of 2026-05-07). One-time discount of USD 1,200 was offered to subscribers prior to 2024-06-01 (valid through 2025-07-01). Payment via PayZang portal. |
...
No state-operated RTK NTRIP caster found for Idaho as of 2026-05-07. ITD (Idaho Transportation Department) installed CORS stations circa 2005–2006 (Pocatello, Idaho Falls, Rexburg, Driggs) for internal use but does not appear to operate a public RTK NTRIP service. Idaho State University's GIS Center documented (giscenter.isu.edu/research/Techpg/GC/rtn.htm) a Real-Time Network for Idaho; the cooperative ISU + Frontier Precision + Monsen + UGRC project established a southeastern Idaho VRS network (Pocatello/ISU + Jerome, Twin Falls, Blackfoot, Idaho Falls + planned Rupert, Aberdeen, Soda Springs) — the network's stations now feed into TURN GPS Utah's coverage footprint.
...
**Gap:** TURN GPS (Utah) extends paid coverage (USD 600/yr) into southern Idaho. EarthScope NOTA provides sparse single-base streams (Plate Boundary Observatory stations). Commercial options: RTKdata (USD 40/mo), Point One Polaris.
...
| **host:port** | `orgn.odot.state.or.us:9881` (network connection; IP 167.131.109.57; confirmed SOURCETABLE 200 OK 2026-05-07). Single-base solutions are also offered on `167.131.0.205:9879` per ODOT Trimble Access PDFs. |
| **tariff** | Free — "All rover users will be issued a rover account at no direct charge" (ODOT Products and Services page, observed 2026-05-07; future subscription fees noted as possible for non-partner users) |
...
| **Operator** | Multi-agency public/private cooperative (WSDOT-led with PANGA/CWU contributing antennae, comms, and data archiving for Puget Sound stations); operated through Trimble Pivot platform; administered from wsrn3.org |
| **host:port** | `wsrn.org:2011` (NAD83-2011 datum) · `wsrn.org:2022` (NATRF2022 new datum); legacy port 8080 is being retired |
| **tariff** | Paid — Non-partner subscription: USD 1,900/yr per login; 5 logins: USD 5,700/yr; 10 logins: USD 10,000/yr; 20 logins: USD 15,000/yr. Partner agencies (government, NGS cooperators) receive free access. Source: WSRN FAQ (wsrn3.org); pricing confirmed in 2015 Caltrans Preliminary Investigation memo (dot.ca.gov real-time-gps-networks-pi-a11y.pdf) and 2016 RPLS forum discussion. Significantly above the $200/yr hobbyist cutoff. |
...
- PANGA (Pacific Northwest Geodetic Array, operated by Central Washington University) operates 220+ GNSS stations in the PNW for geodetic science; real-time GNSS processing is done internally (JPL RTG + Trimble RTKNet); no confirmed public NTRIP caster endpoint found for PANGA as of 2026-05-07. Hobbyists in WA reach the same physical PANGA stations via free EarthScope NOTA (in-pipeline) or — for the WSRN-overlapping subset — via WSRN paid.

---

## HI — Hawaii: No Public State Caster

No state-operated RTK NTRIP caster found for Hawaii as of 2026-05-07. HDOT does not appear to operate a public CORS RTK network. Hawaii is explicitly listed as having no public service in GPS World's December 2024 state-by-state public RTK survey, and is absent from E38 Survey Solutions' state guide. The Pacific GPS Facility at UH SOEST/HIGP (soest.hawaii.edu/pgf/) operates real-time GPS processing for research (constraining KOK1, KOKB, MKEA to ITRF2000) but does not run a public NTRIP caster. The Kīlauea GPS network is an HVO/USGS + UH + Stanford research collaboration, not public.

**Gap:** EarthScope NOTA has a handful of GNSS stations in Hawaii (geologically important volcanic monitoring sites on Big Island and Maui — KOKB, MKEA, MAUI, HILO area), though station spacing is limited and islands are geographically isolated. NPS CORS includes 8 Pacific stations (Hawaii Volcanoes HAVO, Haleakalā HALE) — both flagged offline as of 2026-05-02 per US-NPS file. Topcon announced expansion of Topnet Live service to include Hawaii in October 2024 — a commercial paid option. Commercial alternatives: Topnet Live (Topcon), Point One Polaris. No free VRS network identified.
...
- **Free public NTRIP RTK** exists in 4 of 16 states surveyed (AR, AZ, CO via Mesa County, OR). Plus CA at USD 100 one-time (under the $200/yr cutoff — affordable).
...
- **WSRN's USD 1,900/yr** is the most expensive Western state network — significantly above peers (TURN/Nevada GPS USD 600, MTSRN USD 1,500). LVVWD pricing is undisclosed, requiring direct contact.
- **CRTN's USD 100 one-time** is the cheapest gateway to a large station network (~250 antennae) in the West; falls under the project's $200/yr affordability cutoff.
...
- **Open items requiring direct contact**: ARTGN (NM) operational status; LVVWD (NV) pricing and hobbyist eligibility; AZCORS confirmed port number (Cloudflare CDN obscures); MTSRN biennial rate review for 2026-07-01.
...
- ODOT ORGN connection PDFs (host:port): https://www.oregon.gov/odot/ORGN/Documents/Network-Connection-TSC2-Trimble-Access.pdf, Single-Base-Solutions-TSC2-Trimble-Access.pdf
...
- AZCORS mountpoints doc (Feb 2024): https://www.azwater.gov/sites/default/files/2024-02/AZCORS_InformationAndMountpoints.pdf
...
- LVVWD survey/right-of-way page: https://www.lvvwd.com/engineering-resources/survey-right-of-way/index.html
...
- CRTN station list: https://sopac-csrc.ucsd.edu/index.php/crtn-stationlist/
...
- City of Albuquerque AGRS page: https://www.cabq.gov/municipaldevelopment/architects-engineers-contractors/construction-services/albuquerque-geodetic-reference-system
...
- Topcon Topnet Live expansion (Oct 2024): https://www.topconpositioning.com/us/en/articles/topcon-announces-significant-expansion-of-topnet-live-coverage-across-western-usa-and-hawaii
