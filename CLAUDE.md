...
data/
  ...
  country_markers.json        # Static; country-level markers (122 entries, 5 tiers + vrs flag).
...
presence. `tier` ∈ {`free` (77), `paid` (31), `paid-affordable` (3),
`restricted` (7), `weird` (4)} describes the network's nature for a
hobbyist; an optional `vrs: true` flag (set on 119 of 122) signals VRS /
network-RTK delivery, independent of tier — free SAPOS, paid swipos,
restricted DVRS all carry it. The renderer uses (tier × vrs × data-presence
in `stations.json`) to pick visuals:
- `tier:free + vrs:true + ingested + live` → coloured ring
- `tier:free + vrs:true + ingested-but-stale OR not ingested` → grey ring
- `tier:free + no vrs` (e.g. gpsbru) → grey ring
- `tier:paid|paid-affordable|restricted|weird` → circled-? marker

All five tiers are live in `index.html` via `buildCountryMarkers()`. Toggle
panel shows "VRS networks (N)", "Free, no data yet (N)", "Paid / restricted
(N)" sections.
Design spec in `docs/requirements.md` § Country-level markers; the strict
tier rules and the marker-sweep procedure are codified in the
[`update-country-survey`](.claude/skills/update-country-survey/SKILL.md)
skill (§6 — *country_markers.json sweep*). The self-documenting `_tiers`
and `_vrs_flag` keys at the top of `data/country_markers.json` are the
...
- `free` = no fee. Includes both pipeline-ingested networks and free
  networks whose endpoint is unknown / registration-gated. When no data is
  ingested yet, the `note` must truthfully open with "N stations, free."
- `paid` / `paid-affordable` = substantial national-scale paid commercial
  operator. A small private surveying company with stations in a few
  cities does NOT earn a country-level marker; describe it inline in the
  country prose only.
- `restricted` = substantial national-scale operator with no hobbyist path
  at any price (vetted partners, sector-only, no published rate). If a
  private-user tariff is published, it's `paid` instead.
- `weird` = something unusual overrides the access question for a hobbyist
  (non-standard NTRIP, active jamming/spoofing, infrastructure too sparse
  for RTK to work, war-disrupted with unknown status). Reserved for the
  rare case where a tier alone misleads.
...
6. **(resolved 2026-04-29)** ~~`country_markers.json` "deferred" tier is a
   misnomer — refactor pending.~~ Refactored on this branch
   (`claude/fix-skill-tags-0lohJ`). The marker scheme is now two orthogonal
   axes — `tier` ∈ {`free`, `paid`, `paid-affordable`, `restricted`,
   `weird`} for the network's nature, plus an optional `vrs: true` flag —
   with data presence derived at runtime from `stations.json`. The
   `networks.md` `status:` vocabulary was aligned in the same pass: the
   old `in-pipeline` and `deferred` collapsed into `free`, the four
   genuinely unusual cases (zakpos, igrs, apn, qc_mern) became `weird`,
   and the meta-statuses `candidate` and `rejected` stayed.
