...
from `docs/networks.md`, update `data/country_markers.json` accordingly — add
an `info` marker for paid/restricted entries, a `deferred` marker for free
networks whose endpoint is not yet in the pipeline, or a `vrs` marker for
in-pipeline VRS-only sources. The file is never generated by the workflow.
